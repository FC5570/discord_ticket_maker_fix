...
class DiscordTicket {
  ...
  async resetRole(message) {
    db.delete(`ticketRole_${message.guild.id}`);
    message.channel.send(
      `:white_check_mark: | Successfully reset the ticket role of this server.`
    );
  }
  ...
  async Category(message, ID) {
    ...
    message.channel.send(
      `:white_check_mark: | Successfully set the ticket category to **${ID}**`
    );
  }
  ...
  async makeTicket(message, reason, options) {
    ...
    if (!embedmsg)
      embedmsg =
        "Please wait until the support team is here. They have been notified!";
    ...
    if (message.guild.channels.cache.find((c) => c.id == ot)) {
      ...
      message.channel.send(
        `:x: | You already have a ticket **${channel}** open! Make sure to close it to create another`
      );
    } else {
      ...
      message.guild.channels.create(ticName).then((chan) => {
        ...
        if (ticketRole) {
          ...
          const ROLE = chan.guild.roles.cache.find(
            (r) => r.id == ticketRole.id || r.name == ticketRole.name
          );
          if (ROLE) {
  ...
  async closeTicket(message, channel) {
    ...
    if (ot === "open") {
      channel.delete();
      message.author.send(
        `:white_check_mark: | Ticket - ${channel.name} has been successfully closed!`
      );
    } else {
      return;
    }
